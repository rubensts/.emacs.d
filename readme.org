...
tries to load ~emacs-init.el~ and, in case it doesn't exist yet, loads this ~readme.org~ file.
...
(use-package shackle
  :demand t
  :config
  (validate-setq shackle-default-alignment 'below
...
(use-package langtool
  ...
  (cond ((system-is-linux)
        ...
        ((system-is-mac)
         (validate-setq
          langtool-language-tool-jar "/usr/local/Cellar/languagetool/3.8/libexec/languagetool-commandline.jar")))
